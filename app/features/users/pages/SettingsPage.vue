...
const currentAvatarUrl = computed(
  () =>
    avatarPreviewUrl.value ?? useUserImageUrl(() => profile.value ?? {}).value
);
...
const stopInitWatch = watchEffect(() => {
  if (profile.value && userTagsData.value !== undefined) {
    resetForm({
      values: {
...
const onSubmit = handleSubmit(async values => {
  if (!profile.value || !authUser.value) return;

  try {
    if (values.username !== profile.value.username) {
    ...
    if (avatarFile.value) {
      imageUrl = await uploadUserAvatar(profile.value.id, avatarFile.value);
    }
